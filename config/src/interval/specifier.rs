...
mod specifier_tests {
    ...
    fn filter_out_of_range() {
        ...
        let mut dedupped: Vec<u32> = indices.clone().into_iter().filter(|index| index <= &(max - min)).collect();
        dedupped.sort_unstable();
        dedupped.dedup();
        println!("{:?}", dedupped);
        ...
        let mut dedupped: Vec<u32> = values.clone().into_iter().filter(|value| value <= &max && value >= &min).collect();
        dedupped.sort_unstable();
        dedupped.dedup();
        println!("{:?}", dedupped);
    ...
    #[test]
    fn explicit_nths() {
        let spec = Specifier::new(0 as u32, 10 as u32, SpecifierKind::ExplicitNths(vec![1, 3, 7, 10]));
        assert_eq!(spec.cyclic_next(0), Some(1));
        assert_eq!(spec.cyclic_next(1), Some(3));
        ...
        let spec = Specifier::new(20 as u32, 45 as u32, SpecifierKind::ExplicitNths(vec![0, 5, 10, 11, 15, 20]));
    ...
    #[test]
    fn explicit_list() {
        let spec = Specifier::new(1000 as u32, 1500 as u32, SpecifierKind::ExplicitList(vec![
            1000,
            1075,
            1080,
            1100,
    ...
    #[test]
    fn every_0th() {
        let spec = Specifier::new(25 as u32, 1200 as u32, SpecifierKind::EveryNth(0 as u32, 0 as u32));
        ...
        let spec = Specifier::new(1000 as u32, 1100 as u32, SpecifierKind::EveryNth(0 as u32, 37 as u32));
